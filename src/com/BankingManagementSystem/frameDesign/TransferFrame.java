            ...
            bmanager.addActionListener(new ActionListener() 
	        {
				...
				public void actionPerformed(ActionEvent e) 
				{
					try
					{
						...
						if(tRecAcc.getText().trim().equals( labelAccNo.getText()))
						{
							JOptionPane.showMessageDialog(null, "Receiver and Sender account \n number must be different ");
							tAmount.setText("");
							tRecAcc.setText("");
						}
						else
						{
							...
							if(amt > 0)
							{
								if(tRecAcc.getText().equals(""))
									JOptionPane.showMessageDialog(null, "Please enter a valid Account Number");
								else
								{
									int result = JOptionPane.showConfirmDialog(
										null, "Are you sure?","Confirmation",JOptionPane.INFORMATION_MESSAGE);
									if( result==JOptionPane.OK_OPTION)
									{
...
String message = "Thank you for using Bank India Internation, \n"+tAmount.getText().trim()+" Rupees is successfully transfer from your account to "+tRecAcc.getText().trim();
			 
        	 message = message+" this account.\n" + "Your current balance is "+userlist.get(senIndex).getBalance()+"Rupees";
